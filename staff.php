<?php 
 ...
 include 'koneksi.php';   
  ...
  $res= mysqli_query($conn, "SELECT * FROM mapel");
  if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $sql = mysqli_query($conn, "SELECT * FROM staff WHERE id_staff='$id'");
    if (count($sql) == 1 ) {
      $get = mysqli_fetch_array($sql);
      $nama   = $get['nama'];
      $jk  = $get['jk'];
      $jabatan  = $get['jabatan'];
      $id_mapel  = $get['id_mapel'];
      $alamat  = $get['alamat'];
      $kontak  = $get['kontak'];
      $status  = $get['status'];
      $user  = $get['user'];
              ...
              <div class="box-body">
                <div class="row">
                  <div class="col-md-6">
                   ...
                   <?php
                      if (isset($_GET['id'])) {
                          echo '<form method="post" action="simpan_staff.php?id='.$id.'" enctype="multipart/form-data">';                    
                      } else {
                          echo '<form method="post" action="simpan_staff.php"enctype="multipart/form-data" >';                    
                      }
                    ?>           
                    ...
                    <div class="custom-control custom-radio">
                        <input type="radio" class="custom-control-input" id="Perempuan" name="jk" <?php if($jk=="Perempuan"){echo 'checked';} ?> value="Perempuan"/> 
                        <label class="custom-control-label" for="Perempuan">Perempuan </label>
                    </div>

                    <div class="form-group">
                        <label for="exampleInputEmail1">Jabatan </label>
                        <select type="text" class="form-control" name="Jabatan" placeholder="Pilih Jabatan">
                          <option <?php if($jabatan=="Kepala Sekolah"){ echo 'selected'; } ?> value="Kepala Sekolah">Kepala Sekolah</option>
                        	<option <?php if($jabatan=="Tata Usaha"){ echo 'selected'; } ?> value="Tata Usaha">Tata Usaha</option>
                        	<option <?php if($jabatan=="Guru"){ echo 'selected'; } ?> value="Guru">Guru</option>';
                        </select>
                      </div>

                      <div class="form-group">
                        <label for="exampleInputEmail1">Mata Pelajaran </label>
                        <select type="text" class="form-control" name="mapel" placeholder="Pilih Mata Pelajaran">          
                        
                        <?php
                        while($row=mysqli_fetch_array($res)):;?>
                        <?php 
                          if (isset($_GET['id'])) { ?>
                            <option <?php if($id_mapel==$row[0]){ echo "selected"; } ?> value="<?php echo $row[0];?>">
                                <?php echo $row[1]; ?> 
                              </option>                          
                          <?php } else {
                        ?>
                    ...
                    <div class="form-group purple-border">
                      ...
                      <textarea class="form-control" name="Alamat" id="exampleFormControlTextarea4" rows="3"> <?php echo $alamat; ?> </textarea>
					          </div>
                    ...
                    <div class="custom-control custom-radio">
                        <input type="radio" class="custom-control-input" id="tetap" <?php if($status=="Tetap"){echo 'checked';} ?> name="status" value="Tetap"/> 
                        <label class="custom-control-label" for="tetap">Tetap</label>
                    </div>
                    
                    <div class="custom-control custom-radio">
                        <input type="radio" class="custom-control-input" id="tidak" <?php if($status=="Tidak Tetap"){echo 'checked';} ?> name="status" value="Tidak tetap"/> 
                        <label class="custom-control-label" for="tidak">Tidak Tetap </label>
                    </div>
                    
                    <div class="form-group">
                        <label for="exampleInputEmail1">User </label>
                        <input type="text" class="form-control" name="user" placeholder="User" value="<?php echo $user; ?>">
                    </div>
