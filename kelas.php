<?php 
 include 'koneksi.php'; 
 $kelas="";
 $guru= mysqli_query($conn, "SELECT * FROM staff");
 $siswa= mysqli_query($conn, "SELECT * FROM siswa");
 
    
  if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $sql = mysqli_query($conn, "SELECT * FROM kelas WHERE id_kelas='$id'");
    if (count($sql) == 1 ) {
...
<body id="page-top">
  ...
  <div id="wrapper">
    ...
    <ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar">
      ...
      <!-- Nav Item - Dashboard -->
      <li class="nav-item ">
        <a class="nav-link" href="Dashboard.phpl">
            ...
            <!-- <form method="post" action="addCustomer.php"> -->
              <div class="box-body">
                <div class="row">
                  ...
                  <?php
                      if (isset($_GET['id'])) {
                          echo '<form method="post" action="simpan_kelas.php?id='.$id.'" enctype="multipart/form-data">';                    
                      } else {
                          echo '<form method="post" action="simpan_kelas.php"enctype="multipart/form-data" >';                    
                      }
                    ?>                            
                    ...
                    <div class="form-group">
                        ...
                        <select type="text" class="form-control" name="guru" placeholder="Pilih Mata Pelajaran">
                        <?php
                        while($row=mysqli_fetch_array($guru)):;?>
                        <?php 
                          if (isset($_GET['id'])) { ?>
                            <option <?php if($staff==$row[0]){ echo "selected"; } ?> value="<?php echo $row[0];?>">
                                <?php echo $row[1]; ?> 
                              </option>                          
                          <?php } else {
                        ?>
                    ...
                    <div class="form-group">
                        ...
                        <select type="text" class="form-control" name="siswa" placeholder="Pilih Kelas">
                        <?php
                        while ($rowS=mysqli_fetch_array($siswa)):;?>
                              <?php 
                          if (isset($_GET['id'])) { ?>
                            <option <?php if($NIS==$rowS[0]){ echo "selected"; } ?> value="<?php echo $rowS[0];?>">
                                <?php echo $rowS[1]; ?> 
                              </option>                          
                          <?php } else {
                        ?>
                    ...
                    <div class="box-footer">
                        ...
                        <a href="kelas.php" class="btn btn-warning">Cancel</a>
                    </div>
                    </form>
                  </div>
                </div>        
